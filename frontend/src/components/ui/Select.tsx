...
export const Select: React.FC<SelectProps> = ({
  ...
  const [dropdownPosition, setDropdownPosition] = useState({ top: 0, left: 0, width: 0, maxHeight: 0, flipUp: false });
  ...
  // Update dropdown position when opened with smart flip logic
  const updatePosition = useCallback(() => {
    if (buttonRef.current && isOpen) {
      ...
      // Determine preferred max height (use prop or 320px default)
      const preferredMaxHeight = maxHeight || 320;

      // Decide whether to flip up based on available space
      ...
      const calculatedMaxHeight = shouldFlipUp
        ? Math.min(spaceAbove - 20, preferredMaxHeight)
        : Math.min(spaceBelow - 20, preferredMaxHeight);
  ...
  useEffect(() => {
    if (isOpen) {
      updatePosition();
      // Close dropdown on scroll for better UX (prevents misalignment)
      const handleScroll = () => {
        setIsOpen(false);
        setSearchQuery('');
        setHighlightedIndex(-1);
