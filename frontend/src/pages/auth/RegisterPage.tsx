...
export function RegisterPage({ onSwitchToLogin }: RegisterPageProps) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-theme-bg-primary py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold text-theme-text-primary">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {/* Error Alert */}
          {error && (
            <div className="bg-theme-button-danger-bg border border-theme-button-danger-border rounded-lg p-4">
              <p className="text-sm text-theme-button-danger-text">{error}</p>
            </div>
          )}
          ...
          <div className="space-y-4">
            {/* Name */}
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-theme-text-label mb-1">
            ...
            {/* Email */}
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-theme-text-label mb-1">
            ...
            {/* Password */}
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-theme-text-label mb-1">
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? (
                    <EyeOff className="h-5 w-5 text-theme-text-muted hover:text-theme-text-secondary" />
                  ) : (
                    <Eye className="h-5 w-5 text-theme-text-muted hover:text-theme-text-secondary" />
                  )}
                </button>
              </div>
              ...
              {password && (
                <div className="mt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs">
                    <div className={`flex items-center gap-1 ${passwordStrength.checks.length ? 'text-green-600 dark:text-green-400' : 'text-theme-text-muted'}`}>
                      {passwordStrength.checks.length ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                      8+ characters
                    </div>
                    <div className={`flex items-center gap-1 ${passwordStrength.checks.uppercase ? 'text-green-600 dark:text-green-400' : 'text-theme-text-muted'}`}>
                      {passwordStrength.checks.uppercase ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                      Uppercase
                    </div>
                    <div className={`flex items-center gap-1 ${passwordStrength.checks.lowercase ? 'text-green-600 dark:text-green-400' : 'text-theme-text-muted'}`}>
                      {passwordStrength.checks.lowercase ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                      Lowercase
                    </div>
                    <div className={`flex items-center gap-1 ${passwordStrength.checks.number ? 'text-green-600 dark:text-green-400' : 'text-theme-text-muted'}`}>
            ...
            {/* Confirm Password */}
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-theme-text-label mb-1">
          ...
          <div className="flex items-start">
            <input
              ...
              className="h-4 w-4 mt-0.5 text-blue-600 focus:ring-blue-500 border-theme-border rounded"
              required
            />
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-theme-bg-primary text-theme-text-muted">
